...
function($) {
	...
	$.fn.calendar = function (opt,val) {
		...
		def = $.extend(def, opt);
		var weekDays = 'Dom Seg Ter Qua Qui Sex Sáb'.split(' ');
		...
		var bringMonth = function(p_n,y,m,d) {
			...
			var mon = makeMonth(y,m,d)
				.addClass(p_n)
				.insertBefore(body)
			def.toggle.bind(self)(current.y, current.m, current.d);
		...
		var btn3 = $('<button>')
			...
			.focus(hideYearPicker)
		var btn4 = $('<button data-toggle="modal" data-target="#modal-settings">')
			.addClass('btn ic ic-wrench')
			.ripple()
		...
		var body = makeMonth(current.y, current.m, (opt && opt.date) ? current.d : null)
			.appendTo(month)
		var togg = def.toggle.bind(self)(current.y, current.m, current.d);
		var year = $('<div>')
			.addClass('year-month')
			.prependTo(month)
		var yrnx = $('<button>')
